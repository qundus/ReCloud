...
/**
 * Shows the steps to create and configure an experiment, try maintaining the sequence
 * of the steps because that's how cloudsim is usually initialized.
 */
public class Example  {
  ...
  */
  public static void main(String[] args)  {
    ...
    Datacenters.List().Name("Server").Architecture("x86", "Linux", "Xen").TimeZone(10.0)
    .SecCost(3.0).MemCost(0.05).StorageCost(0.001).BwCost(0.0).Create(1);

    // Hosts.
    Datacenters.Hosts().Mips(177730).Pes(6).Ram(16000).Bw(15000).Storage(4000000)
    .Create(2,"Server");

    // [2]: Create and Configure Brokers, Virtual Machines and Cloudlets (Tasks) with ->
    // Brokers.
    ...
    Brokers.Tasks().Length(10000, 20000).Pes(1).FileSize(1).OutpuSize(1)
    .Create(RandomStyle.Fixed_Pace);
    ...
    Window.SetChartSize(0, 0); // Affects export image size
