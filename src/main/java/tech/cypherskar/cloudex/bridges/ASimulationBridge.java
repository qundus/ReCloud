  ...
  public class ASimulationBridge
  {
    ...
    public void Create(ASimulation... sims)
    {
        ...
        for (int i = 0; i < sims.length; i++) {
          ...
          } else if (sims[i].GetHint() == null || sims[i].GetHint().isEmpty()) {
            sims[i].SetHint(this.hint);
          }
       ...
       * </pre>
       * 
       * The simulation scheduler to be assigned to virtual machines and hosts in the
       * list of schedulers passed in {@link #Create(ASimulation...)}.
       * 
       ...
       * </pre>
       * 
       * The identification title that seperates the simulations in the list passed to
       * {@link #Create(ASimulation...)}, and it shows in the results.
       * 
